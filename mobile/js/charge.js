jQuery(document).ready(function ($) {
	...
	function startup() {
		...
		$(".nav[data-type='" + DefaultChargeKind + "']").addClass('active');
		...
		$('div.container').attr('class', 'container ' + DefaultChargeKind + ' ' + DefaultOperator);
		...
		if (jQuery.inArray(DefaultChargeKind, ['Bill', 'GiftCard', 'Antivirus', 'Support', 'Application', 'InternetPackage']) > -1) {
	...
	$('div.payment-gateways ul li').click(function() {
		$('div.payment-gateways h4 i').text($(this).data('tooltip'));
	...
	$('div#pay-factor').click(function(e) {
	...
	setInterval((function() {
		if (DefaultChargeKind == 'TrafficCard' || DefaultChargeKind == 'GiftCard' || DefaultChargeKind == 'Antivirus') {
			$('input#dataCount').val($('.container.' + DefaultChargeKind + ' .' + DefaultChargeKind + ' #count').val());
		}
		if (DefaultChargeKind == 'PIN') {
			$('input#dataCount').val($('.container.PIN div#content div.charge select#count').val());
		}
		setAmount();
	...
	$('.help').qtip({
		content: {
			text: 'درحال بارگزاری ...',
			ajax: {
				url	: "https://chr724.ir/pages/helpMobile",
				dataType: 'html',
                success: function(data) {
                    this.set('content.text', data);	
                    // $("div.qtip-content").html(data);
                    this.set('position.at', "center");	
                    
                    // console.log($(".mobile-modal").position().top);
                    // $(".mobile-modal").css('top', ($(".mobile-modal").position().top - 1) + 'px');
                    // console.log($(".mobile-modal").position().top);
                }
			},
			title: {
		...
		hide: false,
		style: 'qtip-bootstrap qtip-shadow ui-tooltip-rounded helpModalClass mobile-modal'
	});
	...
	$('.nav').qtip({
		...
		style:
		{
			classes: 'qtip-light qtip-rounded qtip-shadow nav-menu',
		},
		position:
		{
			my: 'bottom center',  // Position my top left...
	...
	function setAmount() {
        if (DefaultChargeKind == 'PIN') {
            $('.container.' + DefaultChargeKind + ' .' + DefaultChargeKind + ' .amount-value').text($('input#dataCount').val() * $('.container.' + DefaultChargeKind + ' .' + DefaultChargeKind + ' input#dataAmount').val());
    ...
    $(".nav.Application").click(function() {
        if (downloadURL == '') {
            $.ajax({
                type: 'POST',
    ...
    $('#download-application').click(function(e) {
        if (downloadURL != '') {
    ...
    $(".nav.Support").click(function() {
        if ($(".yahoo-support a").attr("href") == "#") {
            $(".yahoo-support img").attr("src","http://opi.yahoo.com/online?u=chargereseller24&m=g&t=14");
            $(".yahoo-support a").attr("href","ymsgr:sendim?chargereseller24");
        }
    });
